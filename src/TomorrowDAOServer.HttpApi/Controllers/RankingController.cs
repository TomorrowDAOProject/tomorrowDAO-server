...
public class RankingController : AbpController
{
    ...
    [Authorize]
    public async Task HistoryDataAsync(string chainId, string type, List<string> proposalIds)
    {
        await _rankingAppService.MoveHistoryDataAsync(chainId, type, proposalIds);
    }
