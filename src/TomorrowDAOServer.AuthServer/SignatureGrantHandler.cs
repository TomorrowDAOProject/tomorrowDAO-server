...
public class SignatureGrantHandler : ITokenExtensionGrant
{
    ...
    public virtual async Task<IActionResult> HandleAsync(ExtensionGrantContext context)
    {
        ...
        if (!string.IsNullOrWhiteSpace(caHash))
        {
            ...
            {
                Log.Error("Manager validation failed. caHash:{0}, address:{1}, chainId:{2}",
    ...
    private async Task<List<AddressInfo>> GetAddressInfosAsync(string caHash)
    {
        ...
        _logger.LogInformation("GetAddressInfosAsyncOption chainOptions {0}", JsonConvert.SerializeObject(_chainOptions));
